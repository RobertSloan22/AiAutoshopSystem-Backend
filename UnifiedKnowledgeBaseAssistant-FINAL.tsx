...
export default function UnifiedKnowledgeBaseAssistant({ 
  ...
  const handleFileUpload = async (uploadedFiles: File[]) => {
    ...
    try {
      ...
      if (validFiles.length === 1) {
        // Single file upload - NEW ENDPOINT FORMAT
        ...
        try {
          ...
          // NEW: Include assistantId in URL path
          const uploadResponse = await axiosInstance.post(`/api/assistants/${selectedAssistant.id}/files`, formData, {
            headers: {
        ...
        // Multiple files upload - NEW ENDPOINT FORMAT
        const formData = new FormData();
        ...
        // NEW: Include assistantId in URL path for bulk upload
        const uploadResponse = await axiosInstance.post(`/api/assistants/${selectedAssistant.id}/files/bulk`, formData, {
          headers: {
            'Content-Type': 'multipart/form-data',
          }
        });
  ...
  const fetchAssistantFiles = async (assistantId: string) => {
    try {
      ...
      // NEW: Include assistantId in URL path
      const response = await axiosInstance.get(`/api/assistants/${assistantId}/files`);
