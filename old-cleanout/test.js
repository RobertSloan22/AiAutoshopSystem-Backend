...
// Updated and expanded allowedOrigins list
...
app.use(cors({
  origin: function(origin, callback) {
    ...
    // Important: Add noobtoolai.com to allowed origins if not already included
    if (origin === 'https://noobtoolai.com') {
    ...
    console.log(`CORS request from origin: ${origin} - allowing for testing purposes`);
    // During development/testing, we'll allow all origins with specific origin in response      
    return callback(null, origin);
...
// Add specific CORS headers to all responses as a backup
...
app.use('/api/auth', (req, res, next) => {
  ...
  // Always set specific origin, never use '*' when credentials are involved
  if (origin) {
  ...
  // Handle OPTIONS requests for auth routes - very important for preflight
...
// Special handling for Eliza routes
app.use('/eliza', (req, res, next) => {
  const origin = req.headers.origin;

  if (origin) {
    // Set specific origin for CORS - never use wildcard (*) with credentials
    res.setHeader('Access-Control-Allow-Origin', origin);
    res.setHeader('Access-Control-Allow-Methods', 'GET, POST, PUT, DELETE, OPTIONS, PATCH');     
    res.setHeader('Access-Control-Allow-Headers', 'Content-Type, Authorization, X-Requested-With, Accept, Origin');
    res.setHeader('Access-Control-Allow-Credentials', 'true');
    ...
    console.log(`Eliza CORS request from origin: ${origin}`);
  }

  // Handle OPTIONS requests for Eliza routes
  if (req.method === 'OPTIONS') {
    // Important: Always return 200 for OPTIONS requests
    return res.status(200).end();
  }
...
app.use('/eliza', createProxyMiddleware({
  ...
  onProxyRes: (proxyRes, req, res) => {
    // Handle CORS headers for Eliza specifically
    const origin = req.headers.origin;
    if (origin) {
      // Debug log for CORS responses
      console.log(`Eliza proxy responding to origin: ${origin}`);

      // Remove any existing wildcard CORS headers
  ...
  onError: (err, req, res) => {
    ...
    if (res.writeHead && !res.headersSent) {
      res.writeHead(502);
      res.end("Eliza service is currently unavailable");
    }
  }
}));

// Add specific proxy for Eliza WebSocket connections
app.use('/eliza/ws', createProxyMiddleware({
  target: 'http://localhost:3000',
  changeOrigin: true,
  ws: true,
  pathRewrite: {
    '^/eliza/ws': '/ws' // rewrite path
  },
  onProxyReq: (proxyReq, req, res) => {
    // Log the origin for debugging
    console.log('Eliza WebSocket proxy request from origin:', req.headers.origin);
  },
  onProxyRes: (proxyRes, req, res) => {
    // Handle CORS headers for Eliza WebSockets
    const origin = req.headers.origin;
    if (origin) {
      // Debug log for CORS responses
      console.log(`Eliza WebSocket proxy responding to origin: ${origin}`);
  ...
  onError: (err, req, res) => {
    console.error('Eliza WebSocket proxy error:', err);
    if (res.writeHead && !res.headersSent) {
      res.writeHead(502);
      res.end("Eliza WebSocket service is currently unavailable");
    }
  }
}));

// Proxy requests to research WebSocket server with improved error handling
...
// Proxy requests to Python websocket server with improved error handling
app.use('/ws', createProxyMiddleware({
  ...
  onError: (err, req, res) => {
    ...
    if (res.writeHead && !res.headersSent) {
      res.writeHead(502);
      res.end("WebSocket service is currently unavailable");
    }
  }
}));
...
// Special handling for WebSockets through ngrok
const io = new Server(server, {
  cors: {
    origin: (origin, callback) => {
      // Always allow WebSocket connections from ngrok
      if (origin && (origin.includes('ngrok.app') || origin.includes('ngrok-free.app'))) {       
        callback(null, true);
        return;
      }

      // Otherwise use regular CORS rules
      if (!origin || allowedOrigins.includes(origin) || origin.includes('vercel.app')) {
        callback(null, true);
        return;
      }
...
io.on('connection', (socket) => {
  console.log('WebSocket client connected through ngrok:', socket.id);

  socket.on('disconnect', () => {
    console.log('WebSocket client disconnected:', socket.id);
  });

  // Forward any messages to the appropriate services
  socket.on('message', (data) => {
    console.log('Received message from client:', data);
    // Handle message forwarding here
  });
});
