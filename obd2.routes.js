            ...
            if (toolCall.function.name === 'search_technical_images') {
              try {
                ...
                if (resultContent.success && resultContent.images && resultContent.images.length > 0) {
                  console.log(`📊 CHAT/STREAM: Found ${resultContent.images.length} search result images`);
                  ...
                  console.log(`📊 CHAT/STREAM: Sent image_search_ready event with ${resultContent.images.length} images to frontend`);
                }
              } catch (e) {
  ...
  try {
    ...
    if (result.plots && result.plots.length > 0) {
      ...
      for (const plot of result.plots) {
        // Handle both string paths and plot objects
        const plotPath = typeof plot === 'string' ? plot : plot.path;
        const base64Data = await responsesService.pythonService.getPlotAsBase64(plotPath);
        if (base64Data) {
          result.plots_data.push({
            path: plotPath,
            data: base64Data,
            imageId: typeof plot === 'object' ? plot.imageId : undefined
          });
        }
      }
    }
